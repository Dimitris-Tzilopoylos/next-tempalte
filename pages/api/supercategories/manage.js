...
async function handler(req,res) {
    ...
    const path = process.cwd() + "/public/images/supercategories"
    try {
        if(req.method === "POST") {
            const file_path = `/images/supercategories/${req.body.image.name}`
            ...
            const file_path = image?.name ? `/images/supercategories/${image?.name}` : image
            await db.connect()
